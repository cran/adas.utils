...
Let's see a full example that defines a $3\cdot 2^2$ full factorial plan with factor names and scales, creates a linear model and creates the interaction plot and the response surface for the model, using the factor scales on secondary axes:
...
Now let's create the corresponding response surface:
...
There are thre geometries for creating half-normap probability plot of quantiles:
...
These geometries also work on grouped data:
...
For the general case, when you have a tibble with values and names, use the new `geom_pareto*`:
...
For the case of a linear model, there is a conveniency function `pareto_chart()`:
...
If you prefer, you can rather use a half-normal plot via the conveniency function `daniel_plot_hn`:
...
The `modelr` package has been marked as superseded and it is not actively maintained anymore. It provides useful function, though: `add_predictions()` and `add_residuals()`. These functions have been imported into `adas.utils`, with a slight change: `add_predictions()` also accepts further arguments `...` that are passed down to the underlying `predict()` function. This allows to pass the option `interval = c("none", "confidence", "prediction")`:
